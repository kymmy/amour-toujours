# Le Village — Amour Toujours

**Initiative Solidaire Locale** — Encouraging mutual aid between residents in Marseille 11e & 12e.

**Live:** https://amour-toujours.vercel.app

**Portal:** https://le-village-portal.vercel.app

---

## What is it?

Amour Toujours is a local solidarity initiative that brings neighbours together through shared actions: solidarity dinners, pink collection bins in partner shops, entrepreneur meetups, and charity wine tastings. All proceeds go to supporting families in need.

Includes a dedicated event page for the **Cafe des Entrepreneurs Solidaires** — a monthly morning meetup for expats, entrepreneurs, and remote workers.

## Pages

- `/` — Main dossier: vision, actions, partners, transparency
- `/cafe-entrepreneurs` — Monthly entrepreneur meetup event page

## Tech Stack

- Next.js 16 (App Router)
- TypeScript (strict)
- Tailwind CSS 4
- Framer Motion
- Bilingual FR/EN

## Development

```bash
npm install
npm run dev    # http://localhost:3000
```

## Part of Le Village

| Project | URL | Repo |
|---------|-----|------|
| Portal | [le-village-portal.vercel.app](https://le-village-portal.vercel.app) | [kymmy/le-village-portal](https://github.com/kymmy/le-village-portal) |
| Entourage | [tribu.vercel.app](https://tribu.vercel.app) | [kymmy/tribu-app](https://github.com/kymmy/tribu-app) |
| Little Movers | [little-movers.vercel.app](https://little-movers.vercel.app) | [kymmy/little-movers](https://github.com/kymmy/little-movers) |
| Amour Toujours (this) | [amour-toujours.vercel.app](https://amour-toujours.vercel.app) | This repo |
